...
while True:
    ...
    for i in range(8):
        ...
        print("\n")
    counter=count()
    print("Player 1: "+str(counter[1])+"  Player 2: "+str(counter[0]))
    if(counter[0]==0):
        print("Player 1 wins!!!")
    elif(counter[1]==0):
        print("Player 2 wins!!!")
    ...
    if(checkforlegalmoves(0,1)):
        while(True):
            while True:
                try:
                    x,y=map(int,input("Player 2's turn(0): ").split())
                    break
                except ValueError:
                    print("Please give a proper input")
            if a[x-1][y-1]==".":
                if(validornot(x-1,y-1,0,1)):
                    break
            else:
                print("You are not allowed to rewrite the previous moves")
            print("Not a legal move!!!")
        ...
        print("Player 2, you have no legal moves so your opponent will be playing until you get one.")
        p2=1
    print("    [1] [2] [3] [4] [5] [6] [7] [8]\n")
    for i in range(8):
        print("["+str(i+1)+"]", end="  ")
